---
title: "&#91;OS&#93; Cache and Virtual Memorty"
categories:
  - Lecture Notes
tags:
  - os
...
1. **Explicit read/write system call**
  ...
  - Simple to understand, reasonably efficient for small files
...
2. **Memory-mapped files**
  ...
  곧, 1번처럼 복잡하게 copy를 할 필요 없이, 그냥 phsysical memory를 disk에 대한 write-back cache로 간주하여 direct access를 할 수 있는 것이다.
...
아키를 수강했다면 알겠지만 single instruction을 실행하기 충분한 양의 frame는 computer architecture에 의해 결정된다.
...
이 Process는 아마 곧바로 page fault가 발생할 것이다.

그런데 더 큰 문제점은 page fault로 evict 했던 page가 이전에 사용중이었으므로 그 page를 다시 불러와야 할 가능성이 높고, 그러면 또 다시 page fault가 발생한다.

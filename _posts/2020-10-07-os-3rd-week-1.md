...
따라서, `thread_create()`를 한 이후에 dummy stack frame을 생성해서 마치 thread가 `thread_switch()`에 의해 suspend 된 것처럼 보이도록 하는 것이다.
...
이후 `thread_switch()`에서 `return` 한 후 `thread_yield()`에 의해 suspend된 경우라면 이를 거쳐서,그렇지 않다면 곧바로 예전 코드로 돌아가게 된다.
...
그러면 interrupt handling이 끝날 때 다른 thread로 cpu를 넘기게 되고, 나중에 thread가 resume 하면 `thread_swich()`에서 `return`하여 이전 코드로 되돌아가게 된다.

한 가지 재밌는 점은 state of the interrupted thread가 interrupt에 의해서 한 번, `thread_switch()`에 의해 다시 한 번, 총 두 번 store/restore 된다는 것이다.

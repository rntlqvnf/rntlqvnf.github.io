...
그럼 그 허용되지 않은 instruction이 무엇일까?
...
System call은 프로그램에 의해 의도적으로 만들어진 exception이다.

user mode에서는 권한이 없는 기능을 사용하기 위한 방법이다.

![System call example: opening file][I_8]

위 그림-파일 열기-이 대표적인 예시이다.

#### 2. Exceptions

반대로 Exceptions은 의도치 않게 만들어진 exception을 말한다. 혹은 악의가 있는 코드에 의해 만들어진 exception을 말하기도 한다.

예를 들면 divide-by-zero, page fault, invalid memory access 같은 것들이 있다.

### 2. Asynchronous Exception

반대로 이 exception은 현재 instruction의 실행과 관계가 없는, 상당히 독특한 exception이다.

프로세서의 외부에서 일어난 사건에 의해 프로세서의 interrupt pin이 올라가면서 발생한다. 

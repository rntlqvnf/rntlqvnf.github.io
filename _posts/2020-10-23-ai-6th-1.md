...
단연컨데, 불가능하다. 우리가 Online learning을 하고 있음을 까먹지 말자.
...
가장 간단한 접근법은 그냥 확률에 의존해서 선택하는 것($\epsilon$-greㄷdy)이다.
...
어떤 weight $w_m$에 대해 편미분 한 값은, error가 $w_m$에 대해 증가하는 비율을 의미한다.

따라서 이것의 반대 방향으로 $w_m$을 이동시켜주면 error가 감소할 것이다.
...
이전에 배웠던 iterative improvement(local search)의 hill climbing의 일종이라고 보면 된다.

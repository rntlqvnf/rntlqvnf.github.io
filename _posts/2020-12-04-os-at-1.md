...
- Page table length (numver of pages in segment)
- Access permissions
...
Multilevel Paging을 사용하면 page table tree 가지치기를 사용해서 저장해야 하는 page table의 수를 크게 줄일 수 있다.
...
Inverted Page Table은 physical memory와 같은 크기를 가지며, 각 page frame과 대응하여 오로지 하나의 entry만 가진다.
...
Inverted Page Table은 주소를 PID / Page Number / Offset 으로 자른다.
...
Translation이 끝나면 해당 translation을 TLB에 등록하게 될 것이다. (Cache eviction may occur)

![TLB_3][I_21]

전체적인 흐름은 위와 같다.

TLB는 Page table에 비해 상당히 빨라야만 한다.

그래서 주로 on-chip static memorty로 구현되어 CPU 옆에 위치한다.

요즘 들어서는 성능 향상을 위해 muli-level TLB를 도입하고 있다.

또 한편으로 cache 검색을 빠르게 하기 위해 address comparator의 수가 작아야 한다.

곧, TLB는 set associative로 구현되어야 한다.
...
SW Loaded TLB에서는 TLB miss가 났을 때, HW address translation을 하는 대신 processor가 OS kernel로 trap을 한다.

이 Trap handler에서 kernel이 address lookup을 하고, TLB에 새로운 translation을 load 하는 것이다.

이 방법을 사용하게 되면 OS의 디자인의 자율성이 매우 커진다.

HW에 맞출 필요 없이, OS가 편한대로 page table을 디자인 할 수 있고, 주소 번역도 TLB load도 원하는 대로 제어할 수 있다.

그러나 Trappin을 하는 것 자체가 꽤 많은 사이클을 소모하기 때문에 성능에 있어서 꽤 손해가 생긴다는 단점이 있다.
...
화면을 그리기 위해서 processor는 모든 pixel의 값을 건드려야 한다.

근데 알다시피 pixel의 수는 정말 엄청나게 많다.

그런데 그걸 기존의 TLB 구현으로 시도하면 계속해서 cache eviction이 일어나야만 할 것이다.
...
- **TLB shootdown**
  ...
  Multiprocessor 환경을 생각해보자. 각 processor는 각자의 TLB를 갖는다.
  ...
  이때 해당 TLB를 소유하고 있는 processor 만이 해당 TLB를 수정할 수 있고, 또한 OS는 TLB가 어떤 page를 가지고 있는지 모르기 때문에 모든 processor에게 entry를 지우라는 interrupt를 날려야만 한다.

...
Uninformed 란 말 그대로 정보가 없는 상태에서의 searching 이다.
...
- **Optimal?** : 모든 경로의 cost 가 동일하다면 Optimal 하다. 
...
마찬가지로 왜 Optimal 인지에 대한 증명은 [다음 포스트][P_1]의 A* 증명과 방법이 동일하다.
...
따라서 Time/Space complexity 는 $O(b^{C^{*}/e})$ 가 된다.

그런데 찾아보면 다른 책에서는 $O(b^{1+C^{*}/e})$ 로 소개하고 있다.

...
이런 아이디어에서 적용한 것이 **discounting**이다.
...
예를 들어 living reward가 이고 discount가 0.9일때 optimal policy는 화살표의 방향과 같다.
...
간단하다. Expectimax로 생각하면 된다.

Expectimax에 따라, state는 가능한 action 중 **가장 좋은 결과**를 가져오는 action을 고른다.

따라서 $V$는 가능한 q-value 중 **최댓값**이 된다.

이때 q-value는 단순히 action a를 선택함에 따라 얻어지는 reward의 기댓값을 discounting을 적용하여 계산하는 것에 불과하다.
...
Value iteration은 모든 action에 대해 계산을 해야 되므로 상당히 느리다.

그 대신, 그냥 $\pi$가 정해주는 action에 대해서만 계산을 하면 당연히 더 빨라진다.

![EV EQ][I_29]

식 또한 변하는데, 한 방향만 고려하기 때문에 max를 취할 필요가 없어진다. 
...
이거 가지고 어따 써먹냐는 생각이 들 수 있는데, 생각보다 유용하게 쓰인다. 강화학습이라던가..

## Policy Extraction

Value iteration을 통해 열심히 $V$를 구했다.

그런데 이걸 가지고 어떻게 optimal policy를 뽑아내야 할까?
...
Improvement를 다르게 말하자면 다음 state중에서 가장 높은 value를 가진 state를 선택하는 것이다.

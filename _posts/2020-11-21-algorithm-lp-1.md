...
4. Linear object function을 최대화 또는 최소화 하는 variable-value pair를 찾는다.
   ...
   이를 방지하기 위해 **cancle existing flow path** 개념을 도입해야 한다.

   Cancle exisiting flow path란, 이전에 할당한 flow를 일부 되돌리는 path를 의미한다.
...
눈치채야 할 것은 이런 식으로 두 LP가 성호적인 관계를 이루는(max solution = min solution) 경우가 존재한다는 것이다.
...
이 cut의 존재성을 보이면, 이보다 작은 capacity를 가지는 cut은 존재할 수 없으므로($size(f)\leq capacity(L,R)$) 자연스레 min cut 임이 증명되고, 두 LP의 해가 같음도 증명된다.
...
다시 말해, 모든 capacity를 채웠기 때문에 남은 capacity가 0이 되어서 original edge $e$는 사라지고, reverse edge만 존재한다는 의미이다.

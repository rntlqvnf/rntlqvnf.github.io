...
Race condition이 참 힘든 이유는 버그가 때때로 발생하기 때문이다.

대부분의 경우에서는 맞게 잘 돌아가다가, 가끔 이상한 결과가 나오기도 하니까.
...
곧, shared state에 여러 thread가 정해진 순서 없이 마구잡이로 접근할 수 있다는 얘기이다.

이 덕분에 shared state를 구현할 때에는 가능한 모든 접근 방법을 고려하여 race condition을 제거해야 한다.
...
우리가 오늘 배울 것은 synchronization variables layer의 lock, condition variable, semaphore의 개념이다.
...
이를 위해 내부적으로 잠자고 있는 thread들의 queue가 있으며, 이에 접근하는 3가지 method를 제공한다.
...
그리고 condition variable의 중요한 특성은 **memoryless**하다는 점이다.

이 말은 이전에 signal이 있었건 없었건 무조건 wait은 잠들어야 한다는 얘기이다.

![CV][I_11]

자 그럼 synchronized queue 예제를 수정해보자.

위처럼 queue에 아무것도 없으면 wait을 하고, 다른 thread가 queue에 item을 넣어주면서 signal을 보내면 깨어나서 그 item을 지우게 된다.

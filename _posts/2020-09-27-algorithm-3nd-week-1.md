...
pre-sorting이 되어있으므로 combine 단계에서 $\delta$안의 점들을 모으고 정렬하는데 $O(n)$

따라서 $T(n)\leq 2T(n/2)+O(n)=>T(n)=O(nlogn)$ 이다!
...
놀랍게도, 가능하다! 무려 $O(nlogn)$으로 말이다.

## Another Repres. of polynomials
...
이제 $n\geq 2d+1$개의 distinct point들을 $\pm x_0, \pm x_1, \cdots, \pm x_{n/2-1}$로 잡아보자.

그러면 놀랍게도 $A(x_i)=A_e(x_i^2)+x_iA_o(x_i^2)$와 $A(-x_i)=A_e(x_i^2)-x_iA_o(x_i^2)$는 서로 계산해야 할 것이 겹쳐지게 된다!

...
따라서 MAX는 저 change node가 10을 선택할 것으로 생각하고 탐색을 한다.
...
그래서 구한 방향에 0.8을 주고, 20%는 랜덤으로 선택한다고 했으므로 공평하게 나눠서 각 방향에 0.1씩 주면 될 것이다.

이런 식으로 탐색 안의 탐색은 optimal하긴 하나, 정말 느리다.
...
Preference란 말 그대로 어떤 걸 좋아하는 정도를 의미하고, 선호의 대상을 prize와 lottery라고 한다.
...
그럼 **Rational preference**란 무엇인가?

![Rational][I_28]

이런 조건들을 만족시키는 preference를 rational preference라고 한다.
...
이 조건들을 만족시키 않으면 논리적으로 말이 안되는 상황이 벌어진다.

![INF][I_30]

예를 들어 A보다 B를 좋아하고 B보다 C를 좋아하는데, C보다 A를 좋아해서 A-B-C 사이에서 무한정 반복해서 돌 수도 있다.

![TH][I_29]

여하튼 이렇게 만들어낸 rational preference에 대해서는 위의 수식을 만족시키는 어떤 **real-valued function** $U$가 존재함이 증명되어있다.
...
다만 PPT에서 한 가지 방법을 제시해주고는 있다.

## 3. Human Utility

![Human][I_31]

이는 rational preference의 **continuity**를 이용하는 방식이다.

Prize A의 utility를 구한다고 해보자.

$best\;case \succ A \succ worst\;case$ 니까, **continuity**에 의해 A와 [p,base case, 1-p, worst case]의 선호도가 동등해지는 지점이 있을 것이다.

이 지점을 찾아서, 이때의 $p$를 utility로 정하는 방법이다.
